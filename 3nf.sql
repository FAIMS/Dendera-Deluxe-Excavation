...
select 'insert into ' || replace(aenttypename,' ', '') || ' (uuid, aenttypename, modifiedAt, modifiedBy, createdAt, createdBy, identifier)
...
/*
select 'update '||replace(aenttypename,' ', '') || ' set '||replace(replace(replace(replace(attributename, ' ', '_'),'(',''),')',''),'-','_') || ' = (select response, uuid from valueAsSpreadsheet join latestnondeletedarchent using (uuid) where aenttypeid = ' || aenttypeid || ' and attributeid = ' || attributeid || ');'
...
.separator | 
select 'update '|| replace(aenttypename,' ', '') || ' set ' || replace(replace(replace(replace(replace(attributename, ' ', '_'),'(',''),')',''),'-','_'),'?','_') || ' = ''' || group_concat(replace(response,'''', ''''''),', ') || ''' where uuid = '||  uuid || ';' from valueAsSpreadsheet join latestnondeletedarchent using (uuid) join aenttype using (aenttypeid) where response is not null group by uuid, attributeid;
...
select '
.output Table'|| replace(aenttypename,' ', '') || '.csv 
select * from '||replace(aenttypename,' ', '')||';'
from aenttype;
/*
select '
select * from '||replace(aenttypename,' ', '')||';'
